...
**Prototype rule validation** (the "HTML mockup" question): For SwiftUI, iterate in SwiftUI Previews, never HTML — Apple preview tooling S(2), Hudson A(1), van der Lee A(1), plus the misrepresentation argument (HTML cannot express springs/materials/Dynamic Type, WWDC18 803 S(2)) = 6 pts, 3 orgs → 10/10. For web, an optional throwaway prototype after brief approval — Buxton B(2), NN/g P(2), Knapp B(2) = 6 pts → 10/10.
...
| --- | --- | --- | --- |
| "Apply the 60-30-10 color rule" | 6/10 | Evidence weight 2: no primary source exists — interior-decorating folklore | Rewritten as C3 (accent rarity + Itten's contrast-of-extension + ~3 color families) |
| "Never use pure black #000" (as absolute law) | 7/10 | Concreteness/honesty: comfort guidance, not a standard; Apple ships pure black on OLED | Rewritten as C5 with scope ("on white", dark-mode specifics) |
| "Inner radius = outer radius − gap" (nested radius formula) | 6/10 | Independence: single-catalog sourcing | Merged into L4 (alignment/optical adjustment) |
| "Use the two-step pre-permission dialog" | 5/10 | Contested: Egan's large-scale Pinterest data contradicts uplift claims | Rewritten as O4 — contextual timing, never cold on first launch |
...
| "Skeleton screens always beat spinners" | 6/10 | Overstated: badly built skeletons feel slower | Rewritten as O7 with the content-shaped/static constraint |
| "Onboarding tours: 3 steps = 72% vs 7 steps = 16%" | 4/10 | Vendor benchmark stats with unverifiable methodology | Discarded; replaced by O2 and O3 |
| "Font weights: only 400 + 700" (as single rule) | 6/10 | Independence: effectively single-source (Refactoring UI) | Merged into T6 |
| "Micro-tells list as law (side-tab border, eyebrow pills…)" | 6/10 | Independence: single catalog (Bakaus) | Rewritten as A5 — the practice of automated slop-linting + critique loop |
...
- Named practitioners (11, individually vetted): Frank Rausch ("Modern iOS Navigation Patterns"); Paul Hudson (Hacking with Swift, 8 cited docs); Sarun Wongpatcharapakorn (sarunw.com); Majid Jabrayilov; Antoine van der Lee (avanderlee.com); Marc Edwards (Bjango, pre-2022); Steven Hoober (1,300-user device-grip study, UXmatters 2013); Janum Trivedi (Wave engine, ex-Apple); Nathan Gitter (fluid-interfaces); John Sundell; Erik Kennedy (iOS font guidelines)
...
2. **The AI-slop tell catalog is era-dependent.** The durable rule is "detect convergence," not any frozen tell list — encoded as A2/A5.
3. **Two single-catalog micro-tell claims** (side-tab border, eyebrow pills) remain attributed to one source (Bakaus) inside the prompt's banned list; the checklist item (A5) is the multi-source practice of automated detection.

---

*Reproduced from the source repo's `references/evidence-base.md` (sshahzaiib/senior-designer-skill, MIT). Compiled by the original author; see ATTRIBUTION.md.*
